...
public class BlockChain {
    ...
    static class Block {
        ...
        Block(int num, String prevHash, List<Transaction> txns) {
            ...
            // Calculate initial block hash components (without nonce)
            String initialHash = hash(prevHash + blockNumber + merkleRoot);

            // Find nonce that makes CH end with '0'
            this.nonce = findNonce(initialHash);

            // Final block hash includes the nonce
            this.blockHash = hash(initialHash + this.nonce);
        }

        int findNonce(String initialHash) {
            int nonce = 0;
            while (true) {
                String candidateHash = hash(initialHash + nonce);
                if (candidateHash.charAt(candidateHash.length() - 1) == '0') {
                    return nonce;
                }
                nonce++;
            }
    ...
    public static void main(String[] args) {
        ...
        // Sort transactions by incentive (desc), then receiver (asc), then original order
        allTxns.sort((t1, t2) -> {
            if (t1.incentive != t2.incentive) {
                return Integer.compare(t2.incentive, t1.incentive);
            }
        ...
        // Validate transactions (check sender balance)
        ...
        // Create blocks with up to 4 transactions each
        List<Block> blockchain = new ArrayList<>();
        int blockNum = 1;
        String prevHash = "0";

        for (int i = 0; i < validTxns.size(); i += 4) {
            int end = Math.min(i + 4, validTxns.size());
